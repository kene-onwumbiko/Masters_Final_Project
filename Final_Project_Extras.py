...
new_jan_2020_applications = new_jan_2020_applications.apply(modify_date_category, axis = 1)


# Import the acceptance dataset
jan_2020_acceptance = pd.read_excel(r"C:\Users\keneo\Downloads\Data and Software\Overview_FullData_For_4_Academic_Years - 31 January 2023 - 2020.xlsx",
                                    sheet_name = "Sheet2_New_Acceptance_2")

# Unpivot the acceptance dataset
new_jan_2020_acceptance = jan_2020_acceptance.melt(id_vars = id_vars, value_vars = value_vars, 
                                                    var_name = "Category", 
                                                    value_name = "Number of Acceptances")

# Apply the function to the acceptance dataset
new_jan_2020_acceptance = new_jan_2020_acceptance.apply(modify_date_category, axis = 1)
...
app.layout = html.Div([
    ...
    dcc.Dropdown(
        id='group-filter',
        options=[{'label': group, 'value': group} for group in jan_2020['Group'].unique()],
        value=jan_2020['Group'].unique()[0]
    ),
    ...
    dcc.Dropdown(
        id='school-filter',
        options=[{'label': school, 'value': school} for school in jan_2020['School / Department'].unique()],
        value=jan_2020['School / Department'].unique()[0]
    ),
    ...
    dcc.Dropdown(
        id='level-filter',
        options=[{'label': level, 'value': level} for level in jan_2020['Level'].unique()],
        value=jan_2020['Level'].unique()[0]
    ),
    ...
    dcc.Dropdown(
        id='category-filter',
        options=[{'label': category, 'value': category} for category in jan_2020['Category'].unique()],
        value=jan_2020['Category'].unique()[0]
    ),
...
def update_graph(selected_group, selected_school, selected_level, selected_category):
    filtered_df = jan_2020[
        (jan_2020['Group'] == selected_group) &
        (jan_2020['School / Department'] == selected_school) &
        (jan_2020['Level'] == selected_level) &
        (jan_2020['Category'] == selected_category)
    ]
